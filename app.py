...
def contact():
        if request.method == 'POST':
            ...
            res = pd.DataFrame({'name':name, 'email':email, 'subject':subject ,'message':message,'list':list}, index=[0])
            res.to_csv('./contactusMessage.csv')
            print("The data are saved !")

        
            thistuple = ["Strawberry smoothie",  "banana smoothie", "green smoothie","detox smoothie","Mango smoothie" ,
        "Cherry smoothie","Almond smoothie","Carrot detox smoothie", "Mango juice","Pineapple juice","Orange juice",
        "Apple drink","mixed berries juice"]
        print(thistuple)
...
def add_image():
        if request.method == "POST":
            image = {
                "file_name": request.form.get("cfile_name")
            }
            ...
            return render_template("add_iimage.html")
...
def upload(images):
        if request.method == "POST":
            ...
            if file and allowed_file(images.filename):
                ...
                file.save(os.path.join(app.config['upload_folder'], filename))
                return render_template("upload_images.html",upload=upload, upload_file=upload_file )
